...
The maintainer cannot fund every visitor's API spend. End users supply their own Anthropic, OpenAI, or Gemini key at chat time, held in browser sessionStorage and sent with each request, never persisted server-side. The gate exposes a provider picker that defaults to Anthropic and surfaces Gemini's free tier as the lowest-friction onramp for first-time visitors. The maintainer-funded nightly LLM eval runs behind `workflow_dispatch` only so the cap stays intact. Tradeoff: lower demo conversion than a one-click flow. A landing-page screencast is the fallback for visitors without a key. Operational wiring (header semantics, env var defaults, per-provider routing) lives in `.claude/context/agent.md`.
