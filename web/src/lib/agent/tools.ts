...
const deployDataTools = {
  ...
  searchJobs: tool({
    description:
      'Never fabricate ads, ad ids, or concept ids; resolve every concept id via lookupConcept first. Free-text plus structured live search against the JobTech (Platsbanken) public API. Returns active Swedish ads with description excerpts inline, so you can reason about per-ad fit against the USER PROFILE without a follow-up tool call. At least one of `query` or `occupation_concept_id` is required. Pass `query` with the user\'s profession term in Swedish ("kock" not "chef", "sjuksköterska" not "nurse"). Pass `occupation_concept_id` from a lookupConcept result with type "occupation", and optionally `region` from a lookupConcept result with type "region". Free-text matches reliably; the concept-id filter is narrower and may miss adjacent roles, so prefer keeping `query` set even when you also have an occupation_concept_id. Default top_k is 5.',
