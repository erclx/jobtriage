...
| `bun run screenshots`   | Capture canonical UI states to `.claude/review/screenshots/<host>/`, light and dark per state. Host comes from `SCREENSHOT_BASE_URL` (`localhost` by default). Pass `--check-console-clean` to fail on `console.error` |
