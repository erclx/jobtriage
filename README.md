...
Built with Claude Code as the primary agent, planned through the `.claude/` planning docs ([requirements](.claude/REQUIREMENTS.md), [architecture](.claude/ARCHITECTURE.md)) and gated by the [coding standards](.claude/rules). The full setup is reproducible from the [Claude config](CLAUDE.md).
