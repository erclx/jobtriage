...
| `bun run screenshots`   | Capture canonical UI states to `.claude/review/screenshots/`. Override `SCREENSHOT_BASE_URL` to target a non-localhost origin (outputs under `screenshots/<host>/`). Pass `--check-console-clean` to fail on any captured `console.error`. |
