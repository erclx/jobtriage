...
- Pick a provider (Anthropic, OpenAI, or Gemini) and paste a matching API key. Gemini's free tier covers casual use and is the lowest-friction onramp. The key is held in browser sessionStorage and forwarded to `/api/chat` via a Bearer header, never persisted on disk.
